...
import { forecastTodayData } from "../utils/forecastTodayData";
import { useForecastContext } from "../context/hooks/useForecastContext";
import { forecastTomorrowData } from "../utils/forecastTomorrowData";
...
const SeveralForecasts = () => {
  ...
  if(typeof forecastToday === "string"
    || typeof forecastTomorrow === "string"
    ...
    >
      <div className="flex flex-row items-start gap-4 text-sm">
        {tabs.map((tab) => (
          ...
          ${activeTab === tab ? 'text-violet-500 border-b-violet-500' : 
      ...
      >
        {forecastToday && activeTab === "Today" && (
          forecastToday.map((forecastItem, i) => (
             
             <ForecastCard key={i}
              className={i === forecastToday.length - 1 
                && i === 1 ? "md:border-none" : ""}
             forecastItem={forecastItem}/>
          ))
        )}

        {forecastTomorrow && activeTab === "Tomorrow" && (
          forecastTomorrow.map((forecastItem, i) => (
             
             <ForecastCard key={i}
              className={`${i  === forecastTomorrow.length - 1 ? 
              "md:border-none" : ""}
